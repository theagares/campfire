// Copilot의 window capture 핸들러가 원본 drop을 먼저 받는 회귀를 막는다.
...
(async () => {
  ...
  assert.equal(chatgpt.listeners.window.length, 0);
  assert.equal(chatgpt.listeners.document.length, 1);
  console.log('copilot drop capture ok');
