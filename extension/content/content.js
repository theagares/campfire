...
(function () {
  ...
  function injectFileByDrop(finalFile, preferredTarget, hasEvidence) {
    ...
    for (const target of targets) {
      ...
      // 사이트가 이미 받았다는 증거가 보이면 더 쏘지 않는다. 조상마다 핸들러가 있고
      // 그 핸들러가 event.target 을 확인하지 않으면 같은 문서가 후보 수만큼(실측 5~6개)
      // 중복 첨부된다 — 프롬프트 쪽에서 두 번 고쳤던 그 버그(b125abe, 707c561)의 파일
      // 판이다. 증거가 없을 때는 예전처럼 끝까지 훑으므로 찾던 타깃을 놓치지 않는다.
      if (sent && hasEvidence?.()) break;
    }
    return sent;
  }
   ...
   *  건드리지 않는다(메뉴를 열지 않는다). */
  function injectFileByPaste(finalFile, preferredTarget, hasEvidence) {
    const targets = dropTargets(preferredTarget);
    if (!targets.length) return false;
    let sent = false;
    for (const target of targets) {
  ...
  async function injectFileWithEvidence(finalFile, opts = {}) {
    ...
    await attempt('합성drop', (beginWatch, hasEvidence) => {
      beginWatch();
      return injectFileByDrop(finalFile, dropTarget, hasEvidence);
    });
    ...
    await attempt('합성paste', (beginWatch, hasEvidence) => {
      beginWatch();
      return injectFileByPaste(finalFile, dropTarget, hasEvidence);
    });
    ...
    try {
      if (staged) {
        ...
        if (decision.file?.action === 'upload' && decision.file.maskedBase64) {
          ...
          // 원본을 그대로 다시 넣는 경로다. 마스킹본과 달리 base64ToFile() 을 거치지
          // 않으므로 contentOwnedFiles 에 없다 — 등록하지 않고 합성 drop/paste 를 쏘면
          // 우리 자신의 캡처 리스너가 "처음 보는 파일" 로 보고 삼켜버린다
          // (preventDefault + stopImmediatePropagation). 그러면 폴백 전략이 통째로
          // 무효가 되고, 삼켜진 이벤트가 stageFileAttachment 를 다시 타면서 방금 지운
          // pendingAttachment 가 비동기로 되살아나 다음 프롬프트에 유령 첨부로 붙는다.
          // 마스킹할 게 없는 깨끗한 문서가 오히려 차단되던 원인이다.
          if (staged.file) contentOwnedFiles.add(staged.file);
