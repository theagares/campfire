...
/** 다중 검사의 실제 상태를 패널 상단에 표시한다.
 *
 * 기존에는 renderProgress()가 남긴 `검사 중…`을 다중 결과 이벤트가 갱신하지 않아,
 * 모든 탭이 완료되고 전송 버튼까지 열렸는데도 계속 검사 중인 것처럼 보였다.
 */
function refreshMultiCounts() {
  if (state.kind !== 'multi') return;

  const promptStatus = state.promptMeta?.status || 'pending';
  const stillScanning = state.docs.some(d => d.status === 'pending' || d.status === 'scanning')
    || promptStatus === 'pending' || promptStatus === 'scanning';
  if (stillScanning) {
    el.counts.textContent = '검사 중…';
    return;
  }

  const counted = (c, key) => c?.[key] || 0;
  const pii = state.docs.reduce((n, d) => n + counted(d.counts, 'pii'), 0)
    + counted(state.promptMeta?.counts, 'pii');
  const injection = state.docs.reduce((n, d) => n + counted(d.counts, 'injection'), 0)
    + counted(state.promptMeta?.counts, 'injection');
  const incomplete = promptStatus === 'error'
    || state.docs.some(d => d.status === 'error' || d.status === 'unsupported' || d.status === 'truncated');
  el.counts.textContent = `${incomplete ? '검사 종료' : '검사 완료'} · PII ${pii}건 | INJECTION ${injection}건 탐지`;
}
